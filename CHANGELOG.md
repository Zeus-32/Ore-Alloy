...
### Changed
- Reworked licensing: code is MIT, assets are All Rights Reserved, and `*_small_gear.png` textures are LGPL-3.0-or-later with GregTechCEu attribution.
- Updated textures for dust items to match minecraft `gunpowder-like` style. (`x.x.x-b`)
- Renamed generated Aluminum item IDs and documentation from `aluminium` to `aluminum`; `aluminium` remains accepted as a compatibility alias.
- Diamond now uses the internal `gem` form while keeping the bare `ore_and_alloy:diamond` item ID.

### Added
- Added shaped default dust conversion recipes: dust -> 4 dust piles, dust -> 9 tiny dust piles, 2x2 dust piles -> dust, and 3x3 tiny dust piles -> dust.
- Reworked material form groups for regular metals, hot metals, alloys, hot alloys, and engineering gems.
- Added generated form support for hot ingots, double plates, long rods, small gears, tiny dust piles, dust piles, and gem-based materials.

### Fixed
- Gem materials now include long rods.
- Raw/crushed/ore unification now recognizes direct material names from other mods, such as `raw_zinc` and `crushed_raw_aluminum`, without registering extra Ore & Alloy raw/crushed/ore variants.
...
- Exposed all supported material names as startup-script constants, including `aluminum`, `antimony`, and `wrought_iron`.
...
- Fixed ingot<->nugget crafting generation for aliased material tokens (notably `aluminum/aluminium`) so both conversion recipes are always emitted when forms exist.
